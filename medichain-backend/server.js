...
// CORS configuration (comma-separated via env: CORS_ORIGINS)
const allowedOrigins = (
  process.env.CORS_ORIGINS ||
  "https://medichain.hwkds.dev,https://hwkds.dev,https://www.hwkds.dev,https://hwkds.github.io"
)
  .split(",")
...
// Web3 configuration (local by default, override on Render with RPC_URL)
const rpcUrl =
  process.env.RPC_URL ||
...
if (!contractAddress) {
  console.error(
    "CONTRACT_ADDRESS not set in .env file. Please deploy the contract first.",
  );
  console.error("Set CONTRACT_ADDRESS in environment variables.");
  process.exit(1);
}

const mediChainContract = new web3.eth.Contract(contractABI, contractAddress);

console.log(`Connected to blockchain RPC at ${rpcUrl}`);
...
async function initWeb3() {
  try {
    if (privateKey) {
      ...
      console.log("Using signer from PRIVATE_KEY");
    } else {
      const accounts = await web3.eth.getAccounts();
      if (accounts.length === 0) {
        console.error(
          "No accounts found. Set PRIVATE_KEY for hosted environments.",
        );
        process.exit(1);
      }
      defaultAccount = accounts[0];
      console.log("Using first RPC account (local/dev mode)");
    }

    console.log(`Using account: ${defaultAccount}`);
    console.log(
      `Account balance: ${web3.utils.fromWei(await web3.eth.getBalance(defaultAccount), "ether")} ETH`,
    );
  } catch (error) {
...
app.post("/api/verify", async (req, res) => {
  try {
    ...
    try {
      currentScanCount = parseInt(
        await mediChainContract.methods.scanCounts(qrCode).call(),
      );
    } catch (error) {
    ...
    try {
      await mediChainContract.methods
        .verifyDrug(qrCode, location || "Web Client")
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    try {
      const isAuthorized = await mediChainContract.methods
        .authorizedManufacturers(defaultAccount)
        .call();
      console.log(
        `Manufacturer authorization status for ${defaultAccount}: ${isAuthorized}`,
      );

      if (!isAuthorized) {
        console.log("Authorizing manufacturer before registration...");
        await mediChainContract.methods
          .setManufacturerStatus(defaultAccount, true)
          .send({ from: defaultAccount, gas: 200000 });
...
app.get("/api/drug/:qrCode", async (req, res) => {
  try {
    ...
    const drugDetails = await mediChainContract.methods
      .getDrugDetails(qrCode)
      .call();

    // Get scan count
    const scanCount = await mediChainContract.methods
      .getScanCount(qrCode)
      .call();

    // Check if suspicious
    const suspiciousCheck = await mediChainContract.methods
      .checkIfSuspicious(qrCode)
      .call();
    ...
    res.json({
      ...
      isSuspicious:
        suspiciousCheck.isSuspicious === "true" ||
        suspiciousCheck.isSuspicious === true,
    });
  } catch (error) {
    ...
    if (error.message.includes("not registered")) {
      res
        ...
        .json({
          ...
          message: "No drug registered with this QR code",
        });
    } else {
      res
        .status(500)
        .json({ error: "Error getting drug details", message: error.message });
    }
  }
});
...
async function startServer() {
  ...
  if (!result.success) {
      ...
      if (result.success) {
        ...
        try {
          const configPath = path.join(
            __dirname,
            "../medichain-frontend/public/config.js",
          );
          if (fs.existsSync(configPath)) {
            let configContent = fs.readFileSync(configPath, "utf8");
            configContent = configContent.replace(
              /API_URL = ".*\/api"/,
              `API_URL = "http://localhost:${altPort}/api"`,
            );
    ...
    if (!result.success) {
      console.error(
        "Could not start server on any port. Please check for running processes.",
      );
      process.exit(1);
    }
  }
  ...
  try {
    fs.writeFileSync(
      path.join(__dirname, "current_port.txt"),
